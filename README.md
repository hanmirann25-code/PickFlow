...
측정값 (프로덕션 빌드, 주문 10만 · 주문상품 25만, Chromium 1440×900, 5회)

| 항목                              |      중앙값 |     p95 | 기준    |
| --------------------------------- | ----------: | ------: | ------- |
| 목록 첫 렌더 (메뉴 클릭)          |   **135ms** |   232ms | ≤ 300ms |
| 목록 첫 렌더 (첫 방문, 캐시 없음) |       271ms |   285ms | 참고    |
| 스크롤 프레임                     | **60.2fps** | 60.2fps | ≥ 55fps |
| 필터 적용 응답                    |        59ms |    64ms | ≤ 500ms |
| API 기본 목록                     |      26.6ms |  27.6ms | ≤ 500ms |
| API 필터 조회                     |      19.2ms |  47.4ms | ≤ 500ms |
| API 깊은 페이지(1500쪽)           |     167.3ms | 178.5ms | ≤ 500ms |

측정 방식에서 정해둔 것:

- **평균을 쓰지 않는다.** 한 번 크게 튄 값을 평균이 숨긴다. 중앙값(보통 이 정도)과
  p95(나쁠 때 이 정도)를 함께 본다. **합격 판정은 p95로** 한다.
- **첫 렌더는 두 경로를 따로 잰다.** 콘솔 안에서 메뉴로 들어가는 경우와, 주소로 바로
  들어오는 첫 방문은 성격이 다르다. 섞으면 한 수치로 뭉개진다.
  기준(300ms)은 앞쪽에 적용하고 뒤쪽은 기록만 한다.
- **워밍업 1회는 결과에서 뺀다.** 첫 회는 Next가 그 경로의 청크를 처음 올리고
  커넥션 풀도 그때 연결을 만들어 항상 몇 배 느리다. 그 한 번이 p95를 차지하면
  나머지 측정이 묻힌다. 캐시가 빈 첫 방문 비용은 위 "첫 방문" 항목이 따로 잰다.
